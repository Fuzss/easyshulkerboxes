...
public class EasyShulkerBoxesForgeClient {
    ...
    private static void registerHandlers() {
        ...
        MinecraftForge.EVENT_BUS.addListener((final ScreenEvent.MouseButtonPressed.Pre evt) -> {
            MouseDraggingHandler.INSTANCE.onBeforeMousePressed(evt.getScreen(), evt.getMouseX(), evt.getMouseY(), evt.getButton()).ifPresent(unit -> evt.setCanceled(true));
        });
        MinecraftForge.EVENT_BUS.addListener((final ScreenEvent.MouseDragged.Pre evt) -> {
            MouseDraggingHandler.INSTANCE.onBeforeMouseDragged(evt.getScreen(), evt.getMouseX(), evt.getMouseY(), evt.getMouseButton(), evt.getDragX(), evt.getDragY()).ifPresent(unit -> evt.setCanceled(true));
        });
        MinecraftForge.EVENT_BUS.addListener((final ScreenEvent.MouseButtonReleased.Pre evt) -> {
            MouseDraggingHandler.INSTANCE.onBeforeMouseRelease(evt.getScreen(), evt.getMouseX(), evt.getMouseY(), evt.getButton()).ifPresent(unit -> evt.setCanceled(true));
        });
